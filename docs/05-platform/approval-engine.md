...
### Engine ports (in-process, for module docs' §13)
...
`ApprovalTaskPort.stepTasks(stepId)` — **added 2026-08-10** (`hris-api` inbox module, A-199), the read counterpart, and the channel §13's inbox bullet had been describing as a table read. It returns one step's seats as tasks: `{ instanceId, stepId, requestType, requestId, requesterUserId, requesterName, context, dueAt, tasks: [{ assigneeId, userId, delegateOfUserId, delegateOfName }] }`, or `null` when the step is gone.

Every field is something inbox.md's own contract names and `approval.step.activated` `{ instanceId, stepId, assigneeUserIds }` does not carry: BR-INB-004 fixes `dedupe_key` as **the assignee row id**, §4's `source_ref` wants `requestType` and `requestId`, BR-INB-009 wants `due_at` = `activated_at + sla_hours`, and UC-INB-001 renders a title from the requester and *"request summary fields from the event's context snapshot"*. All of it lives on `approval_instances`, `approval_steps` and `approval_assignees` — this module's tables, which ADR-0001 rule 2 forbids the inbox reading.

**A port rather than a wider payload**, unlike the `requesterUserId` this document added on 2026-08-10 for notification. That was a *recipient*, and this event family already names its recipients in the payload — `assigneeUserIds`, `escalatedToUserIds` — because the consumer needs nothing else. A task snapshot is five facts off three tables that neither of the other two consumers of this event wants, and ADR-0010 states the resolution for exactly that case: *"payloads are pointers — ids and primitives; consumers re-read state by id."* `dueAt` is computed here rather than shipped as two fields because the sum, not the parts, is what BR-INB-009 sorts on, and the SLA lives in the chain snapshot this module owns. `requesterName` and `delegateOfName` come from `employee_directory`, which this module already reads for every timeline name (§13) and which carries nothing encrypted or masked — and they are here rather than resolved by the caller because BR-INB-005 snapshots the rendered title at creation, so a name the inbox fetched a second later would be a second answer to the question this module already answers on its own timeline.

The read is live rather than a snapshot at emit time, and BR-INB-001 is why that is correct: *"the inbox is a navigation layer, never state truth"*. A step decided between emit and handle materializes items that the closure events then close, which is the convergence BR-INB-004's idempotence already promises.
...
- **Inbox:** actionable items materialize from `approval.step.activated` through **`ApprovalTaskPort.stepTasks`** (§7, added 2026-08-10) — `idx_approval_assignees_inbox` is what serves that port from inside this module, and was never a licence for the inbox to read `approval_assignees` itself (ADR-0001 rule 2). Actor completion via `assignee.acted`, sibling and remainder closure on the step and instance events; deep links to module detail screens.
